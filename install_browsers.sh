...
install_helium() {
    ...
    if [ -f "$HOME/Applications/HeliumBrowser.AppImage" ] && [ -f "$HOME/.local/share/applications/helium-browser.desktop" ]; then
        echo -e "${GREEN}Helium Browser AppImage is already installed. Skipping installation.${NC}"
        return
    fi

    echo -e "${GREEN}Installing Helium Browser AppImage...${NC}"
    ...
    echo -e "${CYAN}Fetching latest Helium Browser version...${NC}"
    HELIUM_URL=$(curl -s https://api.github.com/repos/imputnet/helium-linux/releases/latest | grep "browser_download_url.*x86_64.AppImage\"" | grep -v "zsync" | cut -d '"' -f 4)

    if [ -z "$HELIUM_URL" ]; then
        echo -e "${RED}Failed to fetch Helium Browser download URL.${NC}"
        return 1
    fi

    # Download latest Helium Browser AppImage
    echo -e "${CYAN}Downloading Helium Browser AppImage...${NC}"
    if ! wget -O "$HOME/Applications/HeliumBrowser.AppImage" "$HELIUM_URL"; then
        echo -e "${RED}Failed to download Helium Browser AppImage.${NC}"
        return 1
    fi

    # Make it executable
    echo -e "${CYAN}Setting executable permissions...${NC}"
    chmod +x "$HOME/Applications/HeliumBrowser.AppImage"
...
Version=1.0
Name=Helium Browser
Comment=Privacy-focused Chromium-based web browser
GenericName=Web Browser
Keywords=Internet;WWW;Browser;Web;Explorer
Exec=$HOME/Applications/HeliumBrowser.AppImage %u
Terminal=false
Type=Application
Icon=web-browser
Categories=Network;WebBrowser;
StartupNotify=true
MimeType=text/html;text/xml;application/xhtml+xml;application/xml;application/rss+xml;application/rdf+xml;image/gif;image/jpeg;image/png;x-scheme-handler/http;x-scheme-handler/https;x-scheme-handler/ftp;video/webm;application/x-xpinstall;" > "$HOME/.local/share/applications/helium-browser.desktop"

    echo -e "${GREEN}Helium Browser AppImage installation complete!${NC}"
    echo -e "${YELLOW}You can run Helium Browser by executing $HOME/Applications/HeliumBrowser.AppImage or launching it from the applications menu.${NC}"
}
